...
This means that the `PushDappClient` should be used alongside the [Sign SDK](todo) or the
[Auth SDK](todo), via the [Shared Core](todo) setup.
:::

**1. Initialize your WalletConnect Core, using [your Project ID](todo), and pass it to the SDK clients**
...
**Push API is not requried** to send these notifications. To read more about User Action Push Notifications, go [here](todo).
