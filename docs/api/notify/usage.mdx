...
const messageHistory = notifyClient.markNotificationsAsRead({ topic, notificationIds: ["notification-1", "notification-2" ], all: false })

// Will mark all notifications in provided topic as read. When all is true, the notificationIds array is ignored.
const messageHistory = notifyClient.markNotificationsAsRead({ topic, notificationIds: [], all: true })
```
