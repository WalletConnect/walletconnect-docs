...
if(notifyClient.isRegistered({account})) {
  return ;
}

const domain =  'app.mydomain.com', // pass the domain (i.e. the hostname) where your dapp is hosted.
const isLimited =  true // The user will be prompted to authorize this dapp to send and receive messages on their behalf for this domain using their WalletConnect identity.

const siweMessage = notifyClient.formatSiweMessage({
  account,
  domain,
  isLimited
});

const signature = await ethersWallet.signMessage(siweMessage);
