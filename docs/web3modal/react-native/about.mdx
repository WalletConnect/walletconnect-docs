...
```
npx expo install @react-native-async-storage/async-storage react-native-get-random-values react-native-svg react-native-modal @react-native-community/netinfo
```
...
2. Create a file named `expo-shim.js`
...
3. Import `expo-shim.js` in your App root file
...
- Make sure this line executes before the first `@web3modal/wagmi-react-native` import.
...
Make sure you import `@web3modal/wagmi-react-native` before `wagmi` to avoid any issues.
