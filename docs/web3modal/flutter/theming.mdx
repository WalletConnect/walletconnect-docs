...
Finally, `Web3ModalTheme` comes with default border radiuses but you can override these values by passing your own values to `Web3ModalRadiuses()` object and then adding this object to `Web3ModalThemeData`'s `radiuses:` parameter. You can also set no corner radiuses at all by setting this value to `Web3ModalRadiuses.square`:
...
final _themeData = Web3ModalThemeData(
  ...
  radiuses: Web3ModalRadiuses.square, // No corner radius, modal will look square
);
```

_**Note**:_ If you don't wrap your `MaterialApp` with a `Web3ModalTheme` widget, the `Web3Modal` will use the default light theme.
