...
Alternatively you can set the `themeMode` after creating the modal
...
You can also get the current `themeMode` value by calling the `getThemeMode` function
...
Alternatively you can set the `themeVariables` after creating the modal
...
You can also get the current `themeVariables` value by calling the `getThemeVariables` function
...
<Table
  ...
  data={[
    ...
    },
    {
      ...
      description: 'The base border radius for fonts.',
      type: { code: 'string' }
    }
  ]}
/>
