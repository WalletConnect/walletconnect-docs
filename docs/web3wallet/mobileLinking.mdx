...
Either you are a dApp requesting a session approval or a sign request to a wallet or a Wallet redirecting back to a dApp after approval or signing the idea is to call the [redirect options passed through PairingMetadata](wallet-usage?platform=flutter#initialization)

Most commong way for Flutter developers is to use `url_launcher` package and call `launchUrlString()`
