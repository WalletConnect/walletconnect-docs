...
### Aprove or Reject Session Proposal
![](/assets/pairing.gif)
...
A session request expiry is defined by a dapp. It's value must be between now() + 5mins and now() + 7 days. After the session request expires the below event is emmited and session request modal should be removed from the app's UI.
...
A connection state event is emitted whenever a connection state has changed. Web3Wallet client tracks Relay wss connection. The connection state is `true` when both Internet connection and Relay are online, and it is set to `false` otherwise.
