...
```kotlin
val appMetadata = /*App Metadata*/
val appDomain = /*App Domain*/ // URI(appMetadata.url).host
...
To allow users to granularly control which notifications they want to receive within system settings, it is recommended to use different notification channels for different types of notifications. For example, one channel for alerts and another channel for campaigns will allow users to turn off campaigns while still receiving alerts.
