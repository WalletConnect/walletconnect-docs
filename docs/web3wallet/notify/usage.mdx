...
[**Researched features**](#researched-features): Features that are in research state and if proved to be useful will be added to the API.

- [Chaning messages as read state](#chaning-messages-as-read-state):
  Marking messages as read or unread across all devices
- [Receiving notifications state changes](#receiving-notifications-state-changes):
  Listening to notifications state changes
...
### Chaning messages as read state

Changing a messages read state allows the user to know that they already seen the message across all synced devices. 
...
val params = Notify.Params.ChangeNotificationsReadState(topic, notificationIds)

NotifyClient.changeNotificationsReadState(
  params,
  onSuccess = {
...
### Receiving notifications state changes

To receive notifications state changes, you can use the `notify_subscriptions_changed` event. This event will be emitted whenever there is a change in the existing notifications after it's marked as read or unread
...
This would be a part of the `NotifyClient.Delegate` implementation. You can use the `onNotifyNotificationsChanged` method to listen to notifications state changes.



```kotlin
override fun onNotifyNotificationsChanged(onNotifyNotificationsChanged: Notify.Event.NotificationsChanged) {
