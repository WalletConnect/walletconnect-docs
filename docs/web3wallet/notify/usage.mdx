...
- [Changing messages read state](#changing-messages-read-state):
...
## Changing messages read state

Changing a messages read state allows the user to know that they already looked at and seen the message. The state of being read is also synced across all devices where the user can see the notification.
...
val params = Notify.Params.ChangeNotificationsReadState(topic, notificationIds, read)
