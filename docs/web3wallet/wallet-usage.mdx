...
- [Register Device Token (iOS)](?platform=ios#register-device-token)
  Enabling Wallet Push Notifcations by registering a device token.
...
As mentioned before, `event.params.generatedNamespaces` is a set of approved namespaces automatically generated for wallet to approve. You can as well pass your hand-writted set of namespaces.
...
To register a wallet to receive WalletConnect push notifications, call `Web3Wallet.registerPushClient` and pass the device token received from the `didRegisterForRemoteNotificationsWithDeviceToken` method in the `AppDelegate`.
