...
As mentioned before, the `SessionProposalEvent` event is emitted when a dapp initiates a new session with your wallet. The event will include an `event` object with information about the dapp and requested namespaces. The wallet should display a prompt for the user to approve or reject the session.
...
To handle a session request, such as `personal_sign`, you would have to register a request handler as mentioned in point 2 of Namespace Builder section.

You would have to register a handler for every chain and method you want to support. So let's say your wallet supports `eip155:1` and `eip155:137`. This would translate to:
...
Once you have your handlers, you can use `web3Wallet.respondSessionRequest()` to respond to the request as follows:

```dart
Future<void> _personalSignHandler(String topic, dynamic parameters) async {
  final requestId = web3Wallet.pendingRequests.getAll().first;
...
This section introduces a novel protocol method that streamlines the process of establishing a Sign session while simultaneously authenticating a wallet using a Sign-In with Ethereum (SIWE) message. This dual functionality provides instant authentication for dApps, opening up new possibilities for immediate user logins. Furthermore, this method allows dApps to request authorization to perform actions on behalf of the wallet user. These requested permissions are explicitly described to the user within the SIWE message. Consequently, dApps can leverage these authenticated messages to obtain authorization and perform specific actions on behalf of the user.
