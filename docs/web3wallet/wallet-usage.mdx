...
Links to sections on this page. Some sections are platform specific and are only visible when the platform is selected. To view a summary of platform specific sections, check out **Extra (Platform Specific)** under this section.
...
**[Extra (Platform Specific)](#extra-platform-specific)**: Additional information/usage for specific platforms

- Web
  - [Emitting Session Events](?platform=web#emitting-session-events)
    Custom Session Events to emit from the Wallet
- iOS
  - [Subscribe for Web3Wallet Publishers](?platform=ios#subscribe-for-web3wallet-publishers)
    Publishers available to subscribe to for Web3Wallet
- Android
  - [Web3Wallet.WalletDelegate](?platform=android#web3walletwalletdelegate)
    Setting and overriding functions through Web3Wallet delegate. Also includes instructions about VerifyContext.
  - [Format Message](?platform=android#format-message)
    Receiving formatted SIWE message
  - [Register Device Token](?platform=android#register-device-token)
    Enabling Wallet Push Notifcations
- Flutter
  - [To Test](?platform=flutter#register-device-token)
    Running tests using `flutter test`
  - [Useful Commands](?platform=flutter#useful-commands)
    List of useful commands for Flutter
- React Native
  - [Emitting Session Events](?platform=react-native#emitting-session-events-1)
    Custom Session Events to emit from the Wallet
