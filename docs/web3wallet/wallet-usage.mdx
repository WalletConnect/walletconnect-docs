...
Web3Wallet.respondAuthRequest(Wallet.Params.AuthRequestResponse.Result(request.id, signature, issuer)) { error ->
    Log.e("Wallet respond approve", error.throwable.stackTraceToString())
}
```

Respond without giving sdk the private key:
...
Web3Wallet.respondAuthRequest(Wallet.Params.AuthRequestResponse.Result(request.id, cacaoSignature, issuer)) { error ->
    Log.e("Wallet respond approve", error.throwable.stackTraceToString())
}
```
...
```kotlin
val request: Wallet.Event.AuthRequest = // Request from onAuthRequest
val errorCode: Int = 12001
val errorMessage: String = "User Rejected Request"

Web3Wallet.respondAuthRequest(Wallet.Params.AuthRequestResponse.Error(request.id, errorCode, errorMessage)) { error ->
    Log.e("Wallet respond reject", error.throwable.stackTraceToString())
}
```
