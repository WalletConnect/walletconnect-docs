...
const { acknowledged } = await web3wallet.updateSession({ topic: session.topic, namespaces: updatedNamespaces })
// if you wish to be notified when the dapp acks the update
