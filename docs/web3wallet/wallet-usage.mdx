...
However, if you decide not to use `web3Wallet.registerRequestHandler()` and, instead, you decide to handler session requests by subscribing to the `web3Wallet.onSessionRequest` events then all you have to do is construnct your approved namespaces as follows:
...
- If you decided to use the `registerRequestHandler()` method to register handlers for supported methods, as explained in previous section, then you would use the `generatedNamespaces` object in the `approveSession`
- If you decided to handle session requests by subscribing to the `onSessionRequest` events then you would need to pass your own set of approved namespaces.
...
To handle a session request, such as `personal_sign`, you have to ways and they are mutually exclusive, so, you use either one way or the other:
...
2. The second way is registering a request handler for the methods and chains you want to support. So let's say your wallet supports `eip155:1` and `eip155:137`. This would translate to:
