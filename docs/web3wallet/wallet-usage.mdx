...
To handle a session request, sucha as `personal_sign`, you would have to register a request handler as mentioned in poin 2 of Namespace Builder section.

You would have to register a handler for every chain and method you want to support. So let's say your wallet support `eip155:1` and `eip155:137`, this would a simply code for it:
...
Using `disconnectSession()` alone will make the pairing toppic to persist so it can be re-used until it expires. If you want to disconnect (remove) the pairing topic as well you would have add another call as follows:
...
In order to support session events, such as `chainChanged` or `accountChanged` your would have first to register an event emitter for such events for every chain you want to emit an event for similar to, similar to request handlers.
...
And to emit an event you just call `emitSessionEvent()` as follows:
