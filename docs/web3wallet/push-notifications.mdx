...
Once you have setup Firebase configured, you can obtain the device token by calling `messaging().getToken()`. This unique token is used to identify each device.
...
messaging().onMessage(async notification => {
    ...
    const decryptedMessage = await Web3wallet.notifications.decryptMessage(topic, message);

    /*
    * `decryptedMessage` is JsonRpcRequest object, with the full payload of the incorming request such as method, params, id, etc.
