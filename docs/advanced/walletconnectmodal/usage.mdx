...
1. Add Name of your game (can be anything) and your Project ID to the `Assets/WalletConnectUnity/Resources/WalletConnectProjectConfig` asset.
...
WalletConnectModal.Ready += (sender, args) =>
{
    ...
    WalletConnectModal.SignClient.SessionDeleted += (_, sessionEvent) =>
    {
...
private static async Task GetAccountBalance()
{
    ...
    // Because one session have multiple namespaces, we need to select one.
    // In most cases, especially in games, dapp will use only one namespace.
    var @namespace = session.RequiredNamespaces.First();
