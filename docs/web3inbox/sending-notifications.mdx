...
- `accounts` - A list of [CAIP-10](https://github.com/ChainAgnostic/CAIPs/blob/main/CAIPs/caip-10.md) account IDs for which to send the notification to. Max 500 accounts per request. Also see the [limits](#limits) below.
...
- `POST /notify`:
  ...
  - Each app can call this endpoint 1 time every second with a burst up to 5. Rate limited requests will return a 429 status code.
- `GET /subscribers`
  - Each app can call this endpoint 1 time every second with a burst up to 5. Rate limited requests will return a 429 status code.
