...
In order to authorize the dapp to control subscriptions, the user needs to sign a SIWE message which happens automatically when `register()` is called. Depending on the configuration of `domain` and `isLimited`, a different message is generated.
