...
const {
  ...
  markReturnedNotificationsAsRead
} = useNotifications(
...
// mark notifications currently in `notifications` returned array as read
await markReturnedNotificationsAsRead();
```
...
- **markReturnedNotificationsAsRead**: Mark currently fetched notifications as read. *This does not mark all notifications as read, just the ones that have been fetched and returned here.*.
