...
- **logLevel _(Optional)_:** `"info" | "debug" | "error"`. Controls which logs show up on the console.
...
const subscriptionToDifferentDappFromDifferentAccount = getSubscription(differentAccount, differentDappDomain)
...
Recieve push notifications. Whether on Mobile (React Native) or Web via Firebase, the following needs to be performed.

More information about firebase messaging setup found here: https://firebase.google.com/docs/cloud-messaging
...
// initialize firebaseMessaging per Firebase docs
...
#### References
- `registerWithPushServer`: `(token: string, platform: 'fcm' | 'apns') => void`
...
client.on("notify_message", ({ message }) => {
 // handle new realtime notifications here.
})
```
