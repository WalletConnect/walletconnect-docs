...
const { data: notifications, nextPage, markAllNotificationsAsRead, markNotificationsAsRead } = useNotifications(
...
- **markAllNotificationsAsRead**: Mark currently fetched notifications as read. *This does not mark all notifications on notify server as read, only those available on the client*.
...
const { nextPage, markAllNotificationsAsRead, markNotificationAsRead } = client.pageNotifications(
  notificationsPerPage,
  isInfiniteScroll,
  unreadFirst,
)(onUpdate)
