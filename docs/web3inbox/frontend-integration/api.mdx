...
If you are building a wallet or just need more fine grained control over events, the following methods are available.

**Note**:
- With the react package, the event watcher follows the component's life cycle. When the component is unmounted, it will die.
- With the javascript package, the event watcher needs to be manually stopped when it is no longer needed. Not doing this may cause memory leaks.
...
```ts
useNotifyEvent("notify_subscription", ({subscription, allSubscriptions}) => {
  const newSubscription = subscription;
  const allSubscriptionsIncludingNewOne = allSubscriptions
  
  // add handling for new subscription here
})
```

#### References

- **allSubscriptions:** Array of Notify Subscriptions
- **subscription:** Notify subscription of type:
...
```ts
const stopWatcher = client.onNotifyEvent("notify_subscription", ({subscription, allSubscriptions} => {
  const newSubscription = subscription;
  const allSubscriptionsIncludingNewOne = allSubscriptions
...
// when event watcher needs to be killed
stopWatcher()
```
...
```ts
useNotifyEvent("notify_message", ({message}) => {
...
- **message:** Notify Notification of type:
...
```ts
const stopWatcher = client.onNotifyEvent("notify_message", ({ message } => {
  const newMessage = message;

  // add handling for new message here
})
...
```ts
useNotifyEvent("notify_delete", ({allSubscriptions}) => {
  const subscriptionsWithoutDeletedOne = allSubscriptions;
  
  // add handling for deletion now
})
```
...
```ts
const stopWatching = client.onNotifyEvent("notify_delete", ({ subscriptions } => {
  const newMessage = message;
...
### On notify update: When a subscription gets updated
...
```ts
useNotifyEvent("notify_update", ({subscription, allSubscriptions}) => {
  const updatedSubscription = subscription;
  const allSubscriptionsIncludingUpdatedOne = allSubscriptions
  
  // add handling for updated subscription here
})
```
...
```ts
const stopWatcher = client.onNotifyEvent("notify_update", ({subscription, allSubscriptions} => {
  const newSubscription = subscription;
  const allSubscriptionsIncludingNewOne = allSubscriptions
...
### On notify subscriptions changed: When a subscriptions are externally added, deleted or updated
...
```ts
useNotifyEvent("notify_subscriptions_changed", ({subscriptions}) => {
  const allSubscriptions = subscriptions
  
  // add handling for new state of subscriptions here
})
```

#### References

- **subscriptions:** Array of Notify Subscriptions
- **subscription:** Notify subscription of type:
...
```ts
const stopWatcher = client.onNotifyEvent("notify_subscriptions_changed", ({subscriptions} => {
  const allSubscriptions = subscriptions
  
  // add handling for new subscription state here
})
