...
- **isLimited:** Set to `false` to request access to all of the user's notification subscriptions for all apps. Most apps only need access to their own subscriptions, so they do not need to set this.
