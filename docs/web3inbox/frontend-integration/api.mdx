...
const { data: apps, isLoading, discover } = useDiscoverApps()
...
- **discover:** `() => NotifyApp[]`, refresh `apps` manually and also return updated `apps`.
- **apps:** `NotifyApp[]`
...
const appUrl = new URL(apps[0].url)
client.subscribeToDapp(undefined, appUrl.hostname)
```
