...
1. Under Notify API > Configuration > `DAPP INFORMATION`, fill in the Dapp URL field. This is the domain of your app that you will use to publish notifications.

   <img className="image-margin" src={useBaseUrl('/img/w3i/dapp-info.png')} />

1. Under the same section, next to Notification types, click on the "Add Notification Type" button and add a title, notification image and description for your notification type. This is the type of notification that your app will publish.
...
1. Optionally, you can add a logo and description for your project. This will be shown to users when they subscribe to your app. Click "Save" to save your changes.

1. Take note of your new Notify API Secret. You will need this to publish notifications. This secret
...
1. Next, you will need to host `did.json` file on this page at the `/.well-known/` directory of your public URL.
...
1. Optionally, you can also enable Welcome notifications. These are notifications that are sent to users when they subscribe to your app. This is a great way to welcome users to your app and explain what types of notifications they will receive.
   You can enable this by switching on the toggle switch in the Welcome Notifications section of your WalletConnect Cloud project after adding relevant Notification Type, Title, Body & Link.
   <img className="image-margin" src={useBaseUrl('/img/w3i/welcome.png')} />
1. Once this file has been deployed, switch on the toggle switch in the Notify API section of your WalletConnect Cloud project. If the file is correctly hosted, you should see a success toast: "Notify configuration successfully verified"
