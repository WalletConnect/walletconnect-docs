...
- **Default**: For a quick integration you can use `defaultWagmiConfig` function which wraps Wagmi's [`creatConfig`](https://wagmi.sh/core/api/createConfig) function with predefined configuration.
  This includes WalletConnect, Coinbase and Injected connectors, and our [Blockchain API](../../../../cloud/blockchain-api.mdx) as a [transport](https://wagmi.sh/core/api/createConfig#transports) (if the chain is not support by the Blockchain API it will fallback to the chain's default RPC).
