...
To integrate SIWS with AppKit, you need to configure your Web3Modal client using `Modal.Model.AuthPayloadParams`, which are required to create a SIWS message for the user to sign:
...
To check whether the user has signed a SIWS message, check `onSessionAuthenticateResponse` callback from `Web3Modal.ModalDelegate`:
...
fun onSessionAuthenticateResponse(response: Modal.Model.SessionAuthenticateResponse) {
    ...
    if (response is Modal.Model.SessionAuthenticateResponse.Result) {
        if (response.session != null) {
            ...
            // Authentication successful, but no session created (SIWS-only flow)
        }
    } else {
...
### Fallback to SIWS Over Session Request

If the wallet connecting to your dapp does not support One-Click Auth, the SDK will fallback to the `wc_sessionPropose` method and create a session with the wallet. AppKit will then inform the user that they need to sign a message to prove address ownership. AppKit will send a SIWS request to the wallet, and once the wallet responds with a signed message, use `onSIWSAuthenticationResponse` callback to check the result:

```kotlin
 override fun onSIWSAuthenticationResponse(response: Modal.Model.SIWSAuthenticateResponse) {
    if (response is Modal.Model.SIWSAuthenticateResponse.Result) {
