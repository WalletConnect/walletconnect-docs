...
If the wallet connecting to your dapp does not support 1-Click Auth, the SDK will fallback to the wc_sessionPropose method and create a session with the wallet. Web3Modal will then inform the user that they need to sign a message to prove address ownership. Web3Modal will send a SIWE request to the wallet, and once the wallet responds with a signed message, the SIWEAuthenticationPublisher will publish the result. Subscribe to this publisher to handle the fallback:
